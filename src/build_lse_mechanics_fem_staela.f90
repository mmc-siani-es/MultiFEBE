...
subroutine build_lse_mechanics_fem_staela(kr)
  ...
  integer                        :: kc
  integer                        :: ndof_mat
  real(kind=real64), allocatable :: K(:,:), f_e(:)
  integer                        :: row, kdofe
  ...
  do ks_int=1,region(kr)%n_fe_subregions
    ...
    do ke_int=1,part(fe_subregion(ss_int)%part)%n_elements
      ...
      call assemble_fem_staela_stiffness_matrix(se_int,ndof_mat,K)
      deallocate(K)
      ...
      !
      call assemble_fem_staela_coupling(se_int,0)
...
end subroutine build_lse_mechanics_fem_staela
