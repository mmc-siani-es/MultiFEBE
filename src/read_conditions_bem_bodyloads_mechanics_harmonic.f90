...
subroutine read_conditions_bem_bodyloads_mechanics_harmonic(input_fileunit)
            ...
            select case (region(be_bodyload(i)%region)%type)
              ...
              case (fbem_potential)
                do while (.true.)
                  ...
                  if (trim(word).eq.'a') then
                    kword=kword+1
                    if (kword.gt.n_words) exit
                    word=trim(fbem_extract_word(line,kword))
                    be_bodyload(i)%ctype(1,1)=0
                    read(word,*) be_bodyload(i)%cvalue_c(1,1,1)
                    kword=kword+1
                  else
              ...
              case (fbem_viscoelastic)
                do while (.true.)
                  ...
                  if (trim(word).eq.'fx'.or.trim(word).eq.'f1') then
                    ...
                    read(word,*) be_bodyload(i)%cvalue_c(2,1,1)
                    kword=kword+1
                    ...
                    read(word,*) be_bodyload(i)%cvalue_c(3,1,1)
                    kword=kword+1
                  else
                    call fbem_error_message(error_unit,0,trim(section_name),nl,'syntax error in this section line.')
                  end if
...
end subroutine read_conditions_bem_bodyloads_mechanics_harmonic
