...
\begin{large}
\ttfamily
Copyright {\copyright} 2014-2025 Universidad de Las Palmas de Gran Canaria
\\\\
Jacob D.R. Bordón \\ 
Guillermo M. \'Alamo \\
Juan J. Azn\'arez \\
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item 3 and 6 nodes triangular MITC element (locking-free): MITC3i \cite{lee2004,lee2014}, MITC6a \cite{lee2004}.
            \item 4, 8 and 9 nodes quadrilateral MITC element (locking-free): MITC4 \cite{dvorkin1984}, MITC8 \cite{Bathe1986,jung2013}, MITC9 \cite{Bucalem1993,bathe2003}.
        \end{itemize}
    \end{itemize}
...
\midrule
\hyperref[sec:problem]{\texttt{problem}} & Type of problem, dimension $\mathbb{R}^n$ of the ambient space, and analysis to be performed & mandatory \\
\hyperref[sec:frequencies]{\texttt{frequencies}} & Frequencies for a time harmonic analysis & dependent \\
\hyperref[sec:settings]{\texttt{settings}} & Solver settings & optional \\
\hyperref[sec:export]{\texttt{export}} & Export options and settings & optional \\
\hyperref[sec:bem_formulation_boundaries]{\texttt{bem formulation over boundaries}} & Set BEM collocation strategies & optional \\
\midrule
\multicolumn{3}{c}{\textit{Low-level entities of the model (mesh -- geometric entities)}} \\
\midrule
\hyperref[sec:nodes]{\texttt{nodes}} & Nodes of the mesh                                            & dependant  \\
\hyperref[sec:elements]{\texttt{elements}} & Elements of the mesh                                         & dependant  \\
\hyperref[sec:parts]{\texttt{parts}}           & Parts of the mesh (a connected set of elements of the same intrinsic dimension) & dependant  \\
\midrule
\multicolumn{3}{c}{\textit{High-level entities of the model (physical entities)}} \\
\midrule
\hyperref[sec:materials]{\texttt{materials}}   & Materials to be used in the model & optional \\
\hyperref[sec:cross_sections]{\texttt{cross sections}} & Cross sections of the model. It contains data which complements the geometrical information contained in the mesh. It is required for structural elements. & dependant  \\
\hyperref[sec:regions]{\texttt{regions}}       & Regions (or subdomains) of the model & mandatory  \\
\hyperref[sec:fe_subregions]{\texttt{fe subregions}} & Subregions of FE regions (a part with elements of intrinsic dimension $\mathbb{R}^n$ (solid) or lower (structure))     & dependant  \\
\hyperref[sec:boundaries]{\texttt{boundaries}}    & Boundaries of BE regions (a part with elements of intrinsic dimension $\mathbb{R}^{n-1}$) & dependant  \\
\hyperref[sec:be_bodyloads]{\texttt{be body loads}} & Body loads of BE regions (a part with elements of intrinsic dimension from 0D (point) to $\mathbb{R}^{n}$) & dependant  \\
\midrule
\multicolumn{3}{c}{\textit{Auxiliary entities of the model}} \\
\midrule
\hyperref[sec:internal_points]{\texttt{internal points}} & Internal points of BE regions where the solution is calculated at post-processing & optional  \\
\hyperref[sec:internal_elements]{\texttt{internal elements}} & Internal elements of BE regions where the solution is calculated at post-processing & optional  \\
\hyperref[sec:groups]{\texttt{groups}}          & Group of entities of the same class                            & optional  \\
\midrule
\multicolumn{3}{c}{\textit{Model conditions}} \\
\midrule
\hyperref[sec:symmetry_planes]{\texttt{symmetry planes}}               & Symmetry planes of the model          & optional  \\
\hyperref[sec:conditions_be_boundaries]{\texttt{conditions over be boundaries}} & Boundary conditions of BE boundaries  & dependent \\
\hyperref[sec:conditions_fe_elements]{\texttt{conditions over fe elements}}   & Boundary conditions of FE elements    & optional  \\
\hyperref[sec:conditions_nodes]{\texttt{conditions over nodes}}         & Boundary conditions of BE or FE nodes & optional  \\
\hyperref[sec:incident_waves]{\texttt{incident waves}}                & Input or incident wave fields         & dependant
\end{tabular}
...
\label{sec:bem_formulation_boundaries}
...
\section{Low-level entities of the model (mesh -- geometric entities)}
The low-level entities of the model are the elementary geometric entities (mesh) of the model: nodes, elements and parts. By default, the mesh is read from the input file by writing by hand the sections \texttt{[nodes]}, \texttt{[elements]} and \texttt{[parts]}, as explained below. However, there are other ways to create and read the mesh.
...
\section{High-level entities of the model (physical entities)}
...
\subsection{Section \texttt{materials}}
\label{sec:materials}
...
\subsection{Section \texttt{cross sections}}
\label{sec:cross_sections}
\label{subsec:cross_sections}

In order to define a structural finite element four pieces of information are required:
\begin{enumerate}

  \item \textbf{Material properties}. It defines the constitutive law, density, and other intrinsic material properties. When needed by the structural element, these properties are taken from the \textbf{material} assigned to the \textbf{region} where the element belongs.

  \item \textbf{Reference geometry}. It defines the mid-line for beams, mid-surface for plates/shells, etcetera. This information is provided by the defined \textbf{mesh}.
  
  \item \textbf{Structural element type}. It defines the physical behaviour of the element. A given line element could be a simple bar element with only axial tension/compression stiffness, or a spring-dashpot or a beam.
  
  \item \textbf{Cross section properties}. It complements the information provided by the mesh, allowing a full description of the structural element. The cross section properties could be given in a detailed manner (cross section dimensions), or by providing effective geometrical properties (area, inertias, ...), or by providing effective mechanical properties (stiffnesses, masses, ...).
...
\begin{Verbatim}[frame=single, fontsize=\footnotesize, label={general format of section [cross sections]}]
[cross sections]
<# cross sections>
...
...
\end{Verbatim} 
where \texttt{<FE subregions>} in reality denotes \texttt{<\# FE subregions> <list of FE subregions ids.>}.

The list of available types of structural elements are:
\begin{itemize}
  \item Discrete element:
  \begin{itemize}
    \item Discrete axial spring-dashpot (\texttt{spring-dashpot}).  
    \item General discrete translational spring-dashpot (\texttt{distra}).
    \item General discrete rotational/translational spring-dashpot (\texttt{disrotra}).
    \item Discrete point mass (\texttt{pmass}).
  \end{itemize}
  \item Bar finite element (\texttt{bar}).
  \item Beam finite element:
  \begin{itemize}
    \item Straight beam finite element (\texttt{strbeam}).
    \item Curved beam finite element based on the degeneration from solid (\texttt{degbeam}).
  \end{itemize}
  \item Shell finite element based on the degeneration from solid (\texttt{degshell}).
\end{itemize}
When considering a 3D analysis, most of the structural elements require orienting their cross section, which leads to a particular cartesian local basis (local axes $x'y'z'$) where the relevant dimensions and the internal stress resultants are defined. In the case of one-dimensional structural elements, such as discrete general springs-dashpots or beams, the orientation of the $y'$ axis is defined with the help of an auxiliary reference vector $\mathbf{y}'_\mathrm{ref}$, see Figure \ref{fig:local_axes_yref_1d}, where the final $y'$ axis is located on the plane defined by $\mathbf{y}'_\mathrm{ref}$ and the $x'$ axis. Note that the $x'$ axis is defined by the tangent direction of the element mid-line. In the case of two-dimensional structural elements, i.e. plates and shells, the procedure is similar, see Figure \ref{fig:local_axes_yref_2d}, but in this case the $x'$ axis is defined by using an auxiliary reference vector $\mathbf{x}'_\mathrm{ref}$, being the final $x'$ axis on the tangent plane defined by the $z'$ axis (coincident with the normal vector at the point) and the plane defined by $\mathbf{x}'_\mathrm{ref}$ and the $z'$ axis.

\begin{figure}[p]
\input{figures/local_axes_yref.pdf_tex}
\caption{Steps for constructing the local cartesian basis $x'y'z'$ of one-dimensional structural elements from a $y'$ axis reference vector $\mathbf{y}'_\mathrm{ref}$}
\label{fig:local_axes_yref_1d}
\end{figure}

\begin{figure}[p]
\input{figures/local_axes_yref_2d.pdf_tex}
\caption{Steps for constructing the local cartesian basis $x'y'z'$ of two-dimensional structural elements from a $x'$ axis reference vector $\mathbf{x}'_\mathrm{ref}$}
\label{fig:local_axes_yref_2d}
\end{figure}

\subsubsection{Discrete axial spring-dashpot (\texttt{spring-dashpot})}

This type of structural finite element is a conventional axial spring-dashpot element between two nodes, and it is denoted as \texttt{spring-dashpot} in the data section. It is a particular case of \texttt{distra} element type, and much more common. The data introduction is also simpler. Taking into account that \texttt{<\# FE subregions> <list of FE subregions ids.>} is here denoted as \texttt{<FE subregions>}, the syntax to describe this type of elements is:
\begin{itemize}
\item In static analysis, $k_{x'}$ is the spring stiffness:
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for spring-dashpot elements}]
[cross sections]
...
...
\item In time harmonic analysis, the stiffness $k_{x'}$ must be introduced as a complex number to take into account a possible hysteretic damping, and $c_{x'}$ is the viscous damping coefficient:
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for spring-dashpot elements}]
[cross sections]
...
...
\subsubsection{General discrete translational spring/dashpot (\texttt{distra})}

This type of structural finite element introduces a generalized translational spring-dashpot between two nodes, and it is denoted as \texttt{distra} in the data section. It requires defining a 2-node line element. Each element node has 2 or 3 translational degrees of freedom respectively for two- and three-dimensional analyses, see Figure \ref{fig:distra}. For static analysis, stiffnesses in global or local axes are required. For time harmonic analysis, it is required to use complex stiffnesses to introduce stiffnesses (real part) and hysteretic damping (imaginary part), and also viscous damping coefficients.

\begin{figure}[h]
\input{figures/distra.pdf_tex}
\caption{Degrees of freedom and local axis definition of discrete translational springs/dashpots (\textbf{distra})}
\label{fig:distra}
\end{figure}

Mechanical properties can be defined in local or global axes:
\begin{itemize}
    \item \textbf{Local axes}. It allows to establish the stiffnesses and viscous damping coefficients in local axes, i.e. axial spring-dashpot in $x'$ direction ($k_{x'}$, $c_{x'}$) and lateral spring-dashpots in $y'$ and $z'$ directions ($k_{y'}$, $c_{y'}$, $k_{z'}$, $c_{z'}$). In two-dimensional analysis, the local stiffness and damping coefficient matrices are:
    \begin{equation}
    \mathbf{K}'
    =
    \left[
    ...
    \right]
    \end{equation}
    In this latter case, in order to uniquely define the local axis, a reference vector $\mathbf{y}'_\mathrm{ref}$ is required for establishing the local $y'$ axis, see Figures \ref{fig:local_axes_yref_2d} and \ref{fig:distra} for more details.
    
    \item \textbf{Global axes}. It allows to establish the stiffnesses and viscous damping coeficients directly in global axes, i.e. springs and dashpots in $x$, $y$ and $z$ directions: $k_{x}$, $c_{x}$, $k_{y}$, $c_{y}$, $k_{z}$ and, $c_{z}$. Therefore, the relative position of element nodes are unimportant.
    
\end{itemize}

Taking into account that \texttt{<\# FE subregions> <list of FE subregions ids.>} is here denoted as \texttt{<FE subregions>}, the syntax to describe this type of elements is:
\begin{itemize}

\item In a 2D static analysis, this type of elements can be introduced in local or in global axes as follows:
...
\item In a 3D static analysis, this type of elements can be introduced in local (requires $\mathbf{y}'_\mathrm{ref}$) or in global axes as follows:
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for distra elements}]
[cross sections]
...
...
\item In a 2D time harmonic analysis, this type of elements can be introduced in local or in global axes as follows (note that $k_{x'}$ and $k_{y'}$ must be complex numbers):
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for distra elements}]
[cross sections]
...
...
\item In a 3D time harmonic analysis, this type of elements elements can be introduced in local (requires $\mathbf{y}'_\mathrm{ref}$) or in global axes as follows (note that $k_{x'}$, $k_{y'}$ and $k_{z'}$ must be complex numbers):
\begin{Verbatim}[frame=single, fontsize=\footnotesize, label={format of section [cross sections] for distra elements}]
[cross sections]
...
...
\subsubsection{General discrete rotational/translational spring-dashpot (\texttt{disrotra})}

This type of structural finite element introduces a generalized translational/rotational spring-dashpot between two nodes, and it is denoted as \texttt{disrotra} in the data section. It requires defining a 2-node line element. Each element node has 3 or 6 degrees of freedom respectively for two- and three-dimensional analyses, see Figure \ref{fig:disrotra}. For static analysis, stiffnesses in global or local axes are required. For time harmonic analysis, it is required to use complex stiffnesses to introduce stiffnesses (real part) and hysteretic damping (imaginary part), and also viscous damping coefficients. This type of element allows modelling a customized beam, a simple spring-dashpo model of the soil-foundation interaction, etcetera.

\begin{figure}[h]
\input{figures/disrotra.pdf_tex}
\caption{Degrees of freedom and local axis definition of discrete translational/rotational springs/dashpots (\textbf{disrotra})}
\label{fig:disrotra}
\end{figure}

Mechanical properties can be defined in local or global axes:
\begin{itemize}
    \item \textbf{Local axes}. It allows to establish the stiffnesses and viscous damping coefficients in local axes:
    \begin{itemize}
      \item Axial ($x'$ translation): $k_{x'}$, $c_{x'}$.
      \item Torsional ($x'$ rotation): $k_{rx'}$, $c_{rx'}$.
      \item Bending on $x'y'$ plane ($y'$ translation -- $z'$ rotation): $k_{y'}$, $c_{y'}$, $k_{rz'}$, $c_{rz'}$, $k_{y'rz'}$, $c_{y'rz'}$.
      \item Bending on $x'z'$ plane ($z'$ translation -- $y'$ rotation): $k_{z'}$, $c_{z'}$, $k_{ry'}$, $c_{ry'}$, $k_{z'ry'}$, $c_{z'ry'}$
    \end{itemize}
    ...
    \begin{array}{ccc|ccc}
    k_{x'} &   0      &         0 & -k_{x'} &       0 &        0  \\
    0      &   k_{y'} & k_{y'rz'} &       0 & -k_{y'} & k_{y'rz'}\\
    0      & k_{y'rz} &   k_{rz'} &       0 & -k_{y'rz} & 2\dfrac{k_{y'rz'}^2}{k_{y'}}-k_{rz'} \\
    \hline
   -k_{x'} &   0      &         0 &  k_{x'} &      0 &        0    \\
        0  & -k_{y'}  &-k_{y'rz'} &       0 & k_{y'} & -k_{y'rz'}  \\
        0  & k_{y'rz'} & 2\dfrac{k_{y'rz'}^2}{k_{y'}}-k_{rz'} & 0 & -k_{y'rz'} & k_{rz'}
    \end{array}
    \right]
    \end{equation}
    \begin{equation}
    ...
    \begin{array}{ccc|ccc}
    c_{x'} &   0      &         0 & -c_{x'} &       0 &        0  \\
    0      &   c_{y'} & c_{y'rz'} &       0 & -c_{y'} & c_{y'rz'}\\
    0      & c_{y'rz} &   c_{rz'} &       0 & -c_{y'rz} & 2\dfrac{c_{y'rz'}^2}{c_{y'}}-c_{rz'} \\
    \hline
   -c_{x'} &   0      &         0 &  c_{x'} &      0 &        0    \\
        0  & -c_{y'}  &-c_{y'rz'} &       0 & c_{y'} & -c_{y'rz'}  \\
        0  & c_{y'rz'} & 2\dfrac{c_{y'rz'}^2}{c_{y'}}-c_{rz'} & 0 & -c_{y'rz'} & c_{rz'}
    \end{array}
    \right]
    \end{equation}    
    In three-dimensional analysis:
...
k_{x'} & 0 & 0 & 0 & 0 & 0 & -k_{x'} & 0 & 0 & 0 & 0 & 0 \\
 & k_{y'} & 0 & 0 & 0 & k_{y'rz'} & 0 & -k_{y'} & 0 & 0 & 0 & k_{y'rz'} \\    
 &  & k_{z'} & 0 & k_{z'ry'} & 0 & 0 & 0 & -k_{z'} & 0 & -k_{z'ry'} & 0 \\    
 &  &  & k_{rx'} & 0 & 0 & 0 & 0 & 0 & -k_{rx'} & 0 & 0 \\    
 & \mathrm{sym} &  &  & k_{ry'} & 0 & 0 & 0 & k_{z'ry'} & 0 & 2\dfrac{k_{z'ry'}^2}{k_{z'}}-k_{ry'} & 0 \\
 &  &  &  &  & k_{rz'} & 0 & -k_{y'rz'} & 0 & 0 & 0 & 2\dfrac{k_{y'rz'}^2}{k_{y'}}-k_{rz'} \\ 
\hline
 &  &  &  &  &  & k_{x'} & 0 & 0 & 0 & 0 & 0 \\  
 &  &  &  &  &  &  & k_{y'} & 0 & 0 & 0 & k_{y'rz'} \\  
 &  &  &  &  &  &  &  & k_{z'} & 0 & k_{z'ry'} & 0 \\
 &  &  &  &  &  &  &  &  & k_{rx'} & 0 & 0 \\
 & \mathrm{sym} &  &  &  &  &  & \mathrm{sym} &  &  & k_{ry'} & 0 \\
 &  &  &  &  &  &  &  &  &  &  & k_{rz'} \\
    \end{array}
    \right]
    \end{equation}
    ...
    In order to uniquely define the local axis, for three-dimensional analysis a reference vector $\mathbf{y}'_\mathrm{ref}$ is required for establishing the local $y'$ axis, see Figures \ref{fig:local_axes_yref_2d} and \ref{fig:distra} for more details.
    
    \item \textbf{Global axes}. It allows to establish the stiffnesses and viscous damping coeficients directly in global axes, and thus the relative position of element nodes are unimportant.
...
\begin{itemize}
\item In a 2D static analysis, this type of elements can be introduced in local or in global axes as follows:
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for disrotra elements}]
[cross sections]
...
...
\item In a 3D static analysis, this type of elements can be introduced in local (requires $\mathbf{y}'_\mathrm{ref}$) or in global axes as follows:
\begin{Verbatim}[frame=single, fontsize=\footnotesize, label={format of section [cross sections] for disrotra elements}]
[cross sections]
...
...
\item In a 2D time harmonic analysis, this type of elements can be introduced in local or in global axes as follows (note that all stiffnesses $k_\square$ must be complex numbers):
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for disrotra elements}]
[cross sections]
...
...
\item In a 3D time harmonic analysis, this type of elements elements can be introduced in local (requires $\mathbf{x}'_\mathrm{ref}$) or in global axes as follows (note that all stiffnesses $k_\square$ must be complex numbers):
\begin{Verbatim}[frame=single, fontsize=\footnotesize, label={format of section [cross sections] for disrotra elements}]
[cross sections]
...
disrotra <FE subregions> local <kx'> <ky'> <kz'> <krx'> <kry'> <krz'> <ky'rz'> <kz'ry'> 
...
\subsubsection{Discrete point mass (\texttt{pmass})}
...
\left[
\begin{array}{cc}
\mathbf{M}_0^{(e)} & \mathbf{M}_1^{(e)} \\
\left(\mathbf{M}_1^{(e)}\right)^T & \mathbf{M}_2^{(e)}
\end{array}
\right]
\cdot
\left[
\begin{array}{cccccc}
u_x^{(i)} & u_y^{(i)} & u_z^{(i)} & \theta_x^{(i)} & \theta_y^{(i)} & \theta_z^{(i)}
\end{array}
\right]^T
\end{equation}
where $\mathbf{M}^{(e)}$ is the generalized mass matrix, which can be divided into three matrices:
\begin{align}
\mathbf{M}_0^{(e)} &= \mathbf{I} \int\limits_{\Omega}\rho\,\mathrm{d}\Omega = 
\left[
\begin{array}{ccc}
M^{(e)} & 0 & 0 \\
0 & M^{(e)} & 0 \\
...
-r_2r_1 & r_1^2+r_3^2 & -r_2r_3 \\
-r_3r_1 & -r_3r_2 & r_1^2+r_2^2
\end{array}
\right]
\,\mathrm{d}\Omega
=\left[
\begin{array}{ccc}
J_{xx}^{(e)} & -J_{xy}^{(e)} & -J_{xz}^{(e)} \\
-J_{xy}^{(e)} & J_{yy}^{(e)} & -J_{yz}^{(e)} \\
-J_{xz}^{(e)} & -J_{yz}^{(e)} & J_{zz}^{(e)}
\end{array}
\right]
\end{align}
where $\Omega$ is the domain of the body $e$, $\rho$ is its the density distribution,  and $r_j=x_j-x_j^{(i)}$ is the position vector of each body point with respect to the reduction point/node $i$. $\mathbf{M}_0$ is the translational inertia (zero moment of mass), $\mathbf{M}_1$ represents the inertia forces and moments due to the mass imbalance (first moments of mass), and $\mathbf{M}_2$ represents the rotational inertia (second moments of mass). If the node $i$ coincides with the center of mass of the body, then $\mathbf{M}_1^{(e)}$ is a zero matrix. If additionally the body has three symmetry planes or if its principal axes coincides with the global axes, then $\mathbf{M}_2^{(e)}$ is a diagonal matrix. For time harmonic analyses with time factor $e^{i\omega t}$, the generalized displacements acceleration becomes:
\begin{equation}
\mathbf{F}_\mathrm{inertial}^{(e)}
=
-
...
In the data input, each mass matrix term $M_{jk}$ have to be defined. In 2D analysis, the mass matrix is a $3\times3$ matrix because of the degrees of freedom involved:
\begin{align}
\mathbf{F}_\mathrm{inertial}^{(e)}
=
\omega^2
...
\left[
\begin{array}{c}
u_x^{(i)} \\ u_y^{(i)} \\ \theta_z^{(i)}
\end{array}
\right]
\end{align}
In 3D analysis, the mass matrix is a $6\times6$ matrix:
\begin{align}
\mathbf{F}_\mathrm{inertial}^{(e)}
=
\omega^2
...
\begin{itemize}
 \item In a \textbf{2D analysis}, you can introduce only the $3$ non-zero diagonal terms for a balanced mass condition, or the full $3\times3$ matrix for a general unbalanced condition:
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for pmass elements}]
[cross sections]
...
...
\end{Verbatim}
 ...
 \item In a \textbf{3D analysis}, you can introduce only the $6$ non-zero diagonal terms for a balanced mass condition, or the full $6\times6$ matrix for a general unbalanced condition:
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for pmass elements}]
[cross sections]
...
...
\subsubsection{Bar finite element (\texttt{bar})}

This type of structural finite element is a simple tension/compression member \cite{onate2013} between two nodes, and it is denoted as \texttt{bar} in the data section. It requires using a 2-node line element in the mesh. Each element node has 2 or 3 translational degrees of freedom respectively for two- and three-dimensional analyses, and the only stress resultant is the axial force $N_{x'}^{(e)}$, see Figure \ref{fig:bar}. For time harmonic analysis,  a consistent mass matrix is used, and its density is taken from the \textbf{material} defined for the corresponding \textbf{fe subregion} and \textbf{region}.

The only required cross section parameter is the area, and it can be introduced in several ways:
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for bar elements}]
[cross sections]
...
...
\begin{figure}[h]
\input{figures/bar.pdf_tex}
\caption{Bar finite element description (\texttt{bar})}
\label{fig:bar}
\end{figure}

\subsubsection{Straight beam finite element (\texttt{strbeam})}

This type of structural finite element is a conventional straight beam finite element, and it is denoted as \texttt{strbeam} in the data section. By default, Timoshenko kinematic assumption (plane cross section after deformation) is assummed for bending, but the simpler Euler-Bernoulli theory is also available (cross section remains plane and perpendicular to the mid-line after deformation). It can be chosen by using as the type of structural element:
\begin{itemize}
 \item Timoshenko beam theory: \texttt{strbeam} or \texttt{strbeam\_t}
 \item Euler-Bernoulli beam theory: \texttt{strbeam\_eb}.
\end{itemize}
The formulation is based on locking-free Timoshenko beam finite element of Friedman \& Kostmatka \cite{friedman1993}, and Euler-Bernoulli assumptions are simply obtained by nullifying the transverse shear strain factor ($\phi=0$). The element is built by superposition of axial, torsional and bending behaviour. Line elements of 2 or 3 nodes can be used in the mesh. For time harmonic analysis, a consistent mass matrix including rotational inertia is used, and its density is taken from the \textbf{material} defined for the corresponding \textbf{fe subregion} and \textbf{region}.

A symmetric cross section is assumed, and the required cross section parameters are:
\begin{itemize}
  \item Area: $A$.
  \item Area moments of inertia: $I_{x'}$, $I_{y'}$, $I_{z'}$.
  \item Shear correction factors: $\kappa_{x'}$, $\kappa_{y'}$, $\kappa_{z'}$. They are used to calculate the effective shear stiffnesses by correcting the cross section parameters: $\kappa_{x'}I_{x'}$ (torsional constant), $\kappa_{y'}A$ ($y'$ transverse shear area), $\kappa_{z'}A$ ($z'$ transverse shear area).
\end{itemize}
In two-dimensional analysis, each node has 3 DOFs ($u_x$, $u_y$, $\theta_z$), whereas in three-dimensional analysis each node has 6 DOFs ($u_x$, $u_y$, $u_z$, $\theta_x$, $\theta_y$, and $\theta_z$), see Figure \ref{fig:strbeam}.

\begin{figure}[h]
\input{figures/strbeam.pdf_tex}
\caption{Degrees of freedom and local axis definition of straight beam finite elements (\textbf{strbeam}). Element with 2 node shown.}
\label{fig:strbeam}
\end{figure}

The data introduction can be summarized as follows:
\begin{itemize}
  \item In a \textbf{2D analysis} under plane strain, a beam can be used to model an infinitely long ($h_{z'}=1$) wall/plate/shell of thickness $h_{y'}$, or an equivalent structural system. It is required the cross section area $A$ (usual application requires $A = h_{y'}\cdot 1$), the inertia $I_{z'}$ (usual application requires $I_{z'} = 1\cdot h_{y'}^3/12$), and the shear correction factor $\kappa_{y'}$ (usual application requires $\kappa_{y'}=5/6$):
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for strbeam elements}]
[cross sections]
...
strbeam <FE subregions> <A> <I_z'> <kappa_y'>
...
\end{Verbatim}
  \item  In a \textbf{3D analysis}, you can introduce a generic cross section, or a pre-defined cross section shape via its dimensions. The reference vector $\mathbf{y}'_\mathrm{ref}$ for defining the $y'$ axis is required (see Figure \ref{fig:local_axes_yref_1d} for more details). Therefore, it can be introduced as follows:
\begin{Verbatim}[frame=single, fontsize=\footnotesize, label={format of section [cross sections] for strbeam elements}]
[cross sections]
...
...
For stress resultants definition, see Figure \ref{fig:stress_resultants_beam} and section about \texttt{degbeam}.

\subsubsection{Beam finite element obtained from the degeneration of the solid (\texttt{degbeam})}

This type of structural finite element is a beam finite element obtained from the degeneration of the solid via Timoshenko kinematic assumption (plane cross section after deformation), and it is denoted as \texttt{degbeam} in the data section. A rectangular cross section of $h_{y'} \times h_{z'}$ is assumed. It requires using a 2, 3 or 4 node line element in the mesh. In order to avoid locking, a selective integration is used by default, but it can be changed in \texttt{element options} data section. For time harmonic analysis,  a consistent mass matrix is used, and its density is taken from the \textbf{material} defined for the corresponding \textbf{fe subregion} and \textbf{region}.

Similarly to \texttt{strbeam}, in two-dimensional analysis each node has 3 DOFs ($u_x$, $u_y$, $\theta_z$), and in three-dimensional analysis each node has 6 DOFs ($u_x$, $u_y$, $u_z$, $\theta_x$, $\theta_y$, and $\theta_z$), see Figure \ref{fig:degbeam}.

\begin{figure}[h]
\input{figures/degbeam.pdf_tex}
\caption{Degrees of freedom and local axis definition of beam finite elements degenerated from solid (\textbf{degbeam}). Element with 3 node shown.}
\label{fig:degbeam}
\end{figure}

The data introduction can be summarized as follows:
\begin{itemize}
  \item In a \textbf{2D analysis}, a plain strain condition is assumed (width is unitary), being the $x'$ and $y'$ axes contained in the plane and only the cross section height $h_{y'}$ is required:
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for degbeam elements}]
[cross sections]
...
degbeam <FE subregions> <hy'>
...
\end{Verbatim}
  \item  In a \textbf{3D analysis}, the cross section dimensions $h_{y'}$ and $h_{z'}$ are required together with the reference vector $\mathbf{y}'_\mathrm{ref}$ for the $y'$ axis (see Figure \ref{fig:local_axes_yref_1d} for more details):
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for degbeam elements}]
[cross sections]
...
...
\,\mathrm{d}A
\end{equation}
and they are shown in Figure \ref{fig:stress_resultants_beam}.

\begin{figure}[p]
\input{figures/stress_resultants_beam.pdf_tex}
\caption{Beam stress resultants: $N_{x'}$, $V_{y'}$, $V_{z'}$, $T_{x'}$, $M_{y'}$, $M_{z'}$}
\label{fig:stress_resultants_beam}
\end{figure}

\subsubsection{Shell finite element obtained from the degeneration of the solid (\texttt{degshell})}

This type of structural finite element is a shell finite element obtained from the degeneration of the solid via Reissner-Mindlin kinematic assumption \cite{ahmad1970}, and it is denoted as \texttt{degshell} in the data section. It requires using a triangular element (3 or 6 nodes) or a quadrilateral element (4, 8 or 9 nodes) in the mesh. By default, the locking-free Mixed Interpolation of Tensorial Components (MITC) family of elements of Bathe and co-workers \cite{dvorkin1984,Bathe1986,Bucalem1993,bathe2003,lee2004,lee2014} with full integration is used: MITC3i, MITC4, MITC6a, MITC8*, MITC9. Standard shell finite element (without MITC) can still be used by denoting them as \texttt{degshell\_std}.

Each element node $i$ has 3 translational DOF ($u_x^{(i)}$, $u_y^{(i)}$, $u_z^{(i)}$), and 2 local rotational DOFs ($\alpha^{(i)}$ and $\beta^{(i)}$ local rotations related to shell bending) or 3 global rotational DOFs ($\theta_x^{(i)}$, $\theta_y^{(i)}$ and $\theta_z^{(i)}$ global rotations). This shell model does not have rotational stiffness in the normal direction, the so called ``drilling'' stiffness, thus an artificial stiffness is added when using 6 DOF \cite{bathe1996} in order to avoid any singularity. There are three situations where 6 DOFs are mandatory: 
\begin{enumerate}
  \item When imposing certain boundary conditions (for example symmetry conditions).
  \item Along edges or joints between two shells with incompatible orientations.
  \item Along joints between more than two shells, at beam-shell joints, and similar singular joints.
\end{enumerate}
% ¿Añadir una figura para explicar esto?
Therefore, in practical problems most element nodes can be 5 DOFs. Although we could have implemented only 6 DOFs element nodes (more homogeneous treatment), we have decided to reduce as much as possible the number of DOF since we do not use a sparse solver (it is not appropriate for boundary element matrices which are dense). Internally, an automatic procedure for selecting 5 DOFs or 6 DOFs is implemented based on the conditions stated above. 
% Falta enlazar con la seccion [fem node options] para asignar manualmente el tipo de nodo (5 o 6 DOF)
For time harmonic analysis,  a consistent mass matrix is used, and its density is taken from the \textbf{material} defined for the corresponding \textbf{fe subregion} and \textbf{region}.

When defining \texttt{degshell} FE subregions, the data format of the cross section definition only requires the shell thickness and the $\mathbf{x}'_\mathrm{ref}$ reference vector for establishing the $x'$ local axis (see Figure \ref{fig:local_axes_yref_2d} for more details):
\begin{Verbatim}[frame=single, fontsize=\small, label={format of section [cross sections] for degshell elements}]
[cross sections]
...
degshell <FE subregions> <thickness> <x'ref x> <x'ref y> <x'ref z>
...
\end{Verbatim}
If the standard shell finite element (without MITC) is needed, substitute \texttt{degshell} by \texttt{degshell\_std}.
...
\left\{
\begin{array}{c}
\left[\int_{-t/2}^{t/2}\int_{-\mathrm{d}y'/2}^{\mathrm{d}y'/2}\sigma_{x'x'} \,\mathrm{d}y'\mathrm{d}z'\right]/\mathrm{d}y' \\
\left[\int_{-t/2}^{t/2}\int_{-\mathrm{d}x'/2}^{\mathrm{d}x'/2}\sigma_{y'y'} \,\mathrm{d}x'\mathrm{d}z'\right]/\mathrm{d}x'\\
\left[\int_{-t/2}^{t/2}\int_{-\mathrm{d}y'/2}^{\mathrm{d}y'/2} \tau_{x'y'}\,\mathrm{d}y'\mathrm{d}z'\right]/\mathrm{d}y' \\
\left[\int_{-t/2}^{t/2}\int_{-\mathrm{d}y'/2}^{\mathrm{d}y'/2}\left(-z'\right)\cdot\sigma_{x'x'}\,\mathrm{d}y'\mathrm{d}z'\right]/\mathrm{d}y' \\
\left[\int_{-t/2}^{t/2}\int_{-\mathrm{d}x'/2}^{\mathrm{d}x'/2}\left(-z'\right)\cdot\sigma_{y'y'} \,\mathrm{d}x'\mathrm{d}z'\right]/\mathrm{d}x' \\
\left[\int_{-t/2}^{t/2}\int_{-\mathrm{d}y'/2}^{\mathrm{d}y'/2}\left(-z'\right)\cdot\tau_{x'y'} \,\mathrm{d}y'\mathrm{d}z'\right]/\mathrm{d}y'\\
\left[\int_{-t/2}^{t/2}\int_{-\mathrm{d}y'/2}^{\mathrm{d}y'/2}\tau_{x'z'} \,\mathrm{d}y'\mathrm{d}z'\right]/\mathrm{d}y' \\
\left[\int_{-t/2}^{t/2}\int_{-\mathrm{d}x'/2}^{\mathrm{d}x'/2}\tau_{y'z'}\,\mathrm{d}x'\mathrm{d}z'\right]/\mathrm{d}x'
\end{array}
\right\}
=
\int_{-t/2}^{t/2}
...
\,\mathrm{d}z'
\end{equation}
and they are shown in Figure \ref{fig:stress_resultants_shell}.

\begin{figure}[p]
\footnotesize
\input{figures/stress_resultants_shell.pdf_tex}
\caption{Shell stress resultants: $N_{x'}$, $N_{y'}$, $N_{x'y'}$, $M_{x'}$, $M_{y'}$, $M_{x'y'}$, $V_{x'}$, $V_{y'}$}
\label{fig:stress_resultants_shell}
\end{figure}
...
\begin{itemize}
  ...
  \begin{minipage}[b]{\linewidth}
    ...
    \begin{minipage}[t]{0.71\linewidth}
      ...
      \caption{Ordinary boundary connected with two regions. Right: exploded view.}
      \label{fig:ordinary_boundary_interface}
      \end{figure}
    \end{minipage}
  \end{minipage}
  ...
  \begin{minipage}[b]{\linewidth}
    ...
    \begin{figure}[H]
      ...
      \caption{Crack-like boundary. Right: exploded view.}
    \end{figure}
  \end{minipage}
\end{itemize}
...
\section{Auxiliary entities of the model}

\subsection{Internal points}
\label{sec:internal_points}
...
\begin{itemize}
\item List of internal points defined by identifiers, region which contains each one, and point coordinates. 
\begin{Verbatim}[frame=single, fontsize=\small, label={general format of section [internal points]}]
[internal points]
<# internal points>
<internal point id> <region id> <x coord.> <y coord> <z coord.> 
...
\end{Verbatim}
\item From the nodes defined in a mesh (see \ref{sec:settings} for details about mesh file format):
\begin{Verbatim}[frame=single, fontsize=\small, label={general format of section [internal points from mesh]}]
...
\begin{itemize}
  \item Create a group containing all nodes or elements.
  \item Create a group containing all nodes or elements belonging of a given part.
  \item Create a group containing the nodes or elements indicated in a list.
  \item Create a group containing all nodes or elements in the interior or exterior of a bounding ball or box.
\end{itemize}
The format for the data section in each case is:
\begin{Verbatim}[frame=single, fontsize=\small, label={general format of section [groups]}]
[groups]
<n = number of groups>
<group id> nodes all 
...
\label{sec:conditions_be_boundaries}
...
\subsection{Section \texttt{conditions over fe elements}}
\label{sec:conditions_fe_elements}

To be documented.

\subsection{Section \texttt{conditions over nodes}}
\label{sec:conditions_nodes}
...
The definition of the local cartesian basis and the stress resultants sign criteria for beams and shells are established according to Section \ref{subsec:cross_sections} (see Figures \ref{fig:local_axes_yref_1d}, \ref{fig:local_axes_yref_2d}, \ref{fig:stress_resultants_beam} and \ref{fig:stress_resultants_shell}).
...
\begin{itemize}
    ...
    \item Beam stress resultants local axes $x'$, $y'$ and $z'$. Only finite elements.
    \item Bar stress resultants. Only finite elements.
    \item Shell stress resultants. Only finite elements.
    \item Shell stress resultants local axes $x'$, $y'$ and $z'$. Only finite elements.
\end{itemize}
...
$ cd build
\end{Verbatim}
    \item Execute cmake taking into account that the CMake configuration file is in the parent folder. The basic command is:
\begin{Verbatim}[frame=single, fontsize=\small]
...
% Ojo, en W11 a Samuel le fallo "gcc"
% Hay que instalar también: pacman -S mingw-w64-x86_64-openmp
