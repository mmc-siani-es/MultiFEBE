...
subroutine assign_default_conditions_bem_bodyloads_mechanics_static
  ...
  do i=1,n_be_bodyloads
    ...
    select case (be_bodyload(i)%coupling)
      ...
      case (fbem_bl_uncoupled)
        ...
        allocate (be_bodyload(i)%cvalue_r(problem%n,1,1))
        be_bodyload(i)%ctype=0    ! constant body load
        be_bodyload(i)%cvalue_r=0 ! null body load
...
end subroutine assign_default_conditions_bem_bodyloads_mechanics_static
