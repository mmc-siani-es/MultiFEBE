...
subroutine assemble_fem_harela_coupling(se_int,mode)
  ...
  complex(kind=real64), allocatable :: t_e(:), f_e(:)

  !
  ! TODO: falta meter el tratamiento para regiones rigidas
  !
  ...
  if (element(se_int)%n_dimension.eq.problem%n) then
    select case (problem%n)
                  ...
                  do kn=1,sd_fe_n_nodes
                    ...
                    if (element(se_int)%subedge_reversion(kd_fe)) then
                      do kdof_col=1,problem%n
                        b_c(row,1)=b_c(row,1)+Qedge(row_local,col_local)*subedge(sd_fe)%cvalue_c(kdof_col,fbem_element_invert(kn,sd_type),2)
                        col_local=col_local+1
                      end do
                    else
                      do kdof_col=1,problem%n
                        b_c(row,1)=b_c(row,1)+Qedge(row_local,col_local)*subedge(sd_fe)%cvalue_c(kdof_col,kn,2)
                        col_local=col_local+1
                      end do
                    end if
                  end do
                        ...
                        do kdof_col=1,problem%n
                          select case (region(boundary(sb_be)%region(1))%type)
                            ...
                            case (fbem_potential)
                              ...
                              ! q_k = p·n_k
                              col=node(sn_be)%col(1,1)
                              if (boundary(sb_be)%region_boundary_reversion(1)) then
                                A_c(row,col)=A_c(row,col)+Qedge(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                              else
                                A_c(row,col)=A_c(row,col)-Qedge(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                              end if
                            ...
                            case (fbem_viscoelastic)
                              ...
                              A_c(row,col)=A_c(row,col)+Qedge(row_local,col_local)
                            ...
                            case (fbem_poroelastic)
                              ! node(sn_be)%col(             0,1): tau
                              ! node(sn_be)%col(problem%n+1+kdof_col,1): t_k
                              ! q_k = -tau·n_k - t_k
                              ! -tau·n_k
                              col=node(sn_be)%col(0,1)
                              if (boundary(sb_be)%region_boundary_reversion(1)) then
                                A_c(row,col)=A_c(row,col)-Qedge(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                              else
                                A_c(row,col)=A_c(row,col)+Qedge(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                              ...
                              ! -t_k
                              col=node(sn_be)%col(problem%n+1+kdof_col,1)
                              A_c(row,col)=A_c(row,col)+Qedge(row_local,col_local)
      ...
      case (3)
        ...
        stop 'not implemented yet' ! Lo mismo que 2D pero con subfaces
  ...
  if (element(se_int)%n_dimension.eq.(problem%n-1)) then
      ...
      select case (element(se_int)%n_dimension)
        ...
        case (1)
          select case (element(se_int)%fe_type)
            ...
            ! -------------------------------------
            ! STRAIGHT EULER-BERNOULLI BEAM ELEMENT
            ! -------------------------------------
                            ...
                            select case (boundary(sb_be)%class)
                              ...
                              case (fbem_boundary_class_ordinary)
                                select case (region(boundary(sb_be)%region(1))%type)
                                  ...
                                  case (fbem_potential)
                                    ...
                                    col=node(sn_be)%col(1,1)
                                    if (boundary(sb_be)%region_boundary_reversion(1)) then
                                      A_c(row,col)=A_c(row,col)+Qmid(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                                    else
                                      A_c(row,col)=A_c(row,col)-Qmid(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                                    end if
                              ...
                              case (fbem_boundary_class_cracklike)
                                select case (region(boundary(sb_be)%region(1))%type)
                                  ...
                                  case (fbem_potential)
                                    ! node(sn_be)%col(1,1): p^+
                                    ! node(sn_be)%col(1,2): p^-
                                    ! q_k = p^+·n_k^+ + p^-·n_k^-
                                    if (boundary(sb_be)%region_boundary_reversion(1)) then
                                      col=node(sn_be)%col(1,1)
                                      A_c(row,col)=A_c(row,col)+Qmid(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                                      col=node(sn_be)%col(1,2)
                                      A_c(row,col)=A_c(row,col)-Qmid(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                                    else
                                      col=node(sn_be)%col(1,1)
                                      A_c(row,col)=A_c(row,col)-Qmid(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
    ...
    !                                ! q_k = - t_k^+ - t_k^-
    !                                col=node(sn_be)%col(problem%n+kdof_col,1)
    !                                A_c(row,col)=A_c(row,col)+LcQmid(row_local,col_local)
                                    !
                                    ! Usando DBIE (para CC mas generales)
                                    !
                                    ! node(sn_be)%col(problem%n+kdof_col,1): t_k^+
                                  ...
                                  case (fbem_poroelastic)
                                    ! node(sn_be)%col(             0,1): tau^+
                                    ! node(sn_be)%col(problem%n+1+kdof_col,1): t_k^+
                                    ! node(sn_be)%col(             0,2): tau^-
                                    ! node(sn_be)%col(problem%n+1+kdof_col,2): t_k^-
                                    ! q_k = - tau^+·n_k^+ - t_k^+ - tau^-·n_k^- - t_k^-
                                    ! - tau^+·n_k^ - tau^-·n_k^-
                                    if (boundary(sb_be)%region_boundary_reversion(1)) then
                                      col=node(sn_be)%col(0,1)
                                      A_c(row,col)=A_c(row,col)-Qmid(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                                      col=node(sn_be)%col(0,2)
                                      A_c(row,col)=A_c(row,col)+Qmid(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                                    else
                                      col=node(sn_be)%col(0,1)
                                      A_c(row,col)=A_c(row,col)+Qmid(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                                    ...
                                    col=node(sn_be)%col(problem%n+1+kdof_col,1)
                                    A_c(row,col)=A_c(row,col)+Qmid(row_local,col_local)
                                    col=node(sn_be)%col(problem%n+1+kdof_col,2)
                                    A_c(row,col)=A_c(row,col)+Qmid(row_local,col_local)
                          ...
                          ! q_k = - T_k^(1) - T_k^(2)
                          ! where
                          !   - Inviscid fluid    : T_k^(j) =  p·n_k^(j)
                          !   - Viscoelastic solid: T_k^(j) = -t_k^(j)
                          !   - Poroelastic media : T_k^(j) = -tau·n_k^(j) - t_k^(j)
                          ...
                          case (fbem_boundary_coupling_be_fe_be)
                            ...
                            ! ========
                            !
                            ! It is the region where the BE boundary is oriented positively
                            ...
                            select case (region(boundary(sb_be)%region(1))%type)
                              ...
                              case (fbem_viscoelastic)
                                ! node(sn_be)%col(problem%n+kdof_col,1): t_k^{region 1}
                                ! q_k += - t_k^(1)
                                col=node(sn_be)%col(problem%n+kdof_col,1)
                                A_c(row,col)=A_c(row,col)+Qmid(row_local,col_local)
                              ...
                              case (fbem_poroelastic)
                                ! node(sn_be)%col(                   1,1): tau^{region 1}
                                ! node(sn_be)%col(problem%n+1+kdof_col,1): t_k^{region 1}
                                ! q_k += - tau^(1)·n_k^(1) - t_k^(1)
                                col=node(sn_be)%col(0,1)
                                A_c(row,col)=A_c(row,col)+Qmid(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                                col=node(sn_be)%col(problem%n+1+kdof_col,1)
                                A_c(row,col)=A_c(row,col)+Qmid(row_local,col_local)
                            ...
                            ! ========
                            !
                            ! It is the region where the BE boundary is reversely oriented

                            select case (region(boundary(sb_be)%region(2))%type)
                              ...
                              case (fbem_viscoelastic)
                                ! node(sn_be)%col(problem%n+kdof_col,2): t_k^{region 2}
                                ! q_k += - t_k^(2)
                                col=node(sn_be)%col(problem%n+kdof_col,2)
                                A_c(row,col)=A_c(row,col)+Qmid(row_local,col_local)
                              ...
                              case (fbem_poroelastic)
                                ! node(sn_be)%col(                   1,2): tau^{region 2}
                                ! node(sn_be)%col(problem%n+1+kdof_col,2): t_k^{region 2}
                                ! q_k += tau^(2)·n_k^(1) - t_k^(2)
                                col=node(sn_be)%col(0,2)
                                A_c(row,col)=A_c(row,col)-Qmid(row_local,col_local)*element(se_be)%n_fn(kdof_col,kn_be)
                                col=node(sn_be)%col(problem%n+1+kdof_col,2)
                                A_c(row,col)=A_c(row,col)+Qmid(row_local,col_local)
                      ...
                      case (fbem_boundary_class_ordinary)
                        select case (region(boundary(sb_be)%region(1))%type)
                          ...
                          case (fbem_potential)
                            ...
                            ! q_k = p·n_k
                            if (boundary(sb_be)%region_boundary_reversion(1)) then
                              t_e((problem%n*(kn-1)+1):(problem%n*kn))= node(sn_be)%value_c(1,1)*element(se_be)%n_fn(:,kn_be)
                            else
                              t_e((problem%n*(kn-1)+1):(problem%n*kn))=-node(sn_be)%value_c(1,1)*element(se_be)%n_fn(:,kn_be)
                            end if
                          ...
                          case (fbem_poroelastic)
                            ...
                            ! q_k = -tau·n_k - t_k
                            if (boundary(sb_be)%region_boundary_reversion(1)) then
                              t_e((problem%n*(kn-1)+1):(problem%n*kn))=-node(sn_be)%value_c(0,1)*element(se_be)%n_fn(:,kn_be)
                            else
                              t_e((problem%n*(kn-1)+1):(problem%n*kn))= node(sn_be)%value_c(0,1)*element(se_be)%n_fn(:,kn_be)
                            end if
                            t_e((problem%n*(kn-1)+1):(problem%n*kn))=t_e((problem%n*(kn-1)+1):(problem%n*kn))&
                                                                    +node(sn_be)%value_c((problem%n+2):(2*problem%n+1),1)
                        end select
                      ...
                      case (fbem_boundary_class_cracklike)
                        select case (region(boundary(sb_be)%region(1))%type)
                          ...
                          case (fbem_potential)
                            ...
                            if (boundary(sb_be)%region_boundary_reversion(1)) then
                              t_e((problem%n*(kn-1)+1):(problem%n*kn))= node(sn_be)%value_c(1,1)*element(se_be)%n_fn(:,kn_be)&
                                                                       -node(sn_be)%value_c(1,2)*element(se_be)%n_fn(:,kn_be)
                            else
                              t_e((problem%n*(kn-1)+1):(problem%n*kn))=-node(sn_be)%value_c(1,1)*element(se_be)%n_fn(:,kn_be)&
                                                                       +node(sn_be)%value_c(1,2)*element(se_be)%n_fn(:,kn_be)
                            end if
...
!~                            t_e((problem%n*(kn-1)+1):(problem%n*kn))=node(sn_be)%value_r((problem%n+1):2*problem%n,1)
                            ...
                            ! q_k = - t_k^+ - t_k^-
                            t_e((problem%n*(kn-1)+1):(problem%n*kn))=node(sn_be)%value_c((problem%n+1):2*problem%n,1)&
                                                                    +node(sn_be)%value_c((problem%n+1):2*problem%n,2)
                          ...
                          case (fbem_poroelastic)
                            ...
                            if (boundary(sb_be)%region_boundary_reversion(1)) then
                              t_e((problem%n*(kn-1)+1):(problem%n*kn))=-node(sn_be)%value_c(0,1)*element(se_be)%n_fn(:,kn_be)&
                                                                       +node(sn_be)%value_c(0,2)*element(se_be)%n_fn(:,kn_be)
                            else
                              t_e((problem%n*(kn-1)+1):(problem%n*kn))= node(sn_be)%value_c(0,1)*element(se_be)%n_fn(:,kn_be)&
                                                                       -node(sn_be)%value_c(0,2)*element(se_be)%n_fn(:,kn_be)
                            end if
                            t_e((problem%n*(kn-1)+1):(problem%n*kn))=t_e((problem%n*(kn-1)+1):(problem%n*kn))&
                                                                    +node(sn_be)%value_c((problem%n+2):(2*problem%n+1),1)&
                                                                    +node(sn_be)%value_c((problem%n+2):(2*problem%n+1),2)
                        end select
...
end subroutine assemble_fem_harela_coupling
