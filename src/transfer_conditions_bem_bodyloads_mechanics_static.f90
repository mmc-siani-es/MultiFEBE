...
subroutine transfer_conditions_bem_bodyloads_mechanics_static
    ...
    select case (be_bodyload(kb)%coupling)
      ...
      case (fbem_bl_uncoupled)
            do kn=1,part(sp)%n_nodes
              ...
              allocate (node(sn)%cvalue_r(problem%n,1,1))
              node(sn)%ctype=be_bodyload(kb)%ctype
              node(sn)%cvalue_r=be_bodyload(kb)%cvalue_r
            end do
...
end subroutine transfer_conditions_bem_bodyloads_mechanics_static
