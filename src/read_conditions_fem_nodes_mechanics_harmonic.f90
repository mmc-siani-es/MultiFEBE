...
subroutine read_conditions_fem_nodes_mechanics_harmonic(input_fileunit)
              ...
              select case (node(kn)%ctype(k,1))
                ...
                case (0,1)
                  ...
                  do kng=2,group(kg)%n_objects
                    ...
                    node(sn)%cvalue_c(k,1,1)=node(kn)%cvalue_c(k,1,1)
                  end do
                ...
                case (4)
                  ...
                  if (k.le.problem%n) then
                    ...
                    do kng=1,group(kg)%n_objects
                      ...
                      node(sn)%ctype(k,1)=0
                      node(sn)%cvalue_c(k,1,1)=urot(k)*theta
                    end do
                  else
    ...
    do ks=1,n_fe_subregions
      ...
      do kn=1,part(sp)%n_nodes
        ...
        if (found) then
          ...
          !!! Cuando se actualice esto, tener en cuenta los DOF de cada nodo individualmente, de acuerdo a lo introducido en [fem node options]
          ! ???? por ahora siempre se leen desplazamientos y rotaciones para todo....
            ...
            select case (node(sn)%ctype(k,1))
              ...
              case (4)
                ...
                if (k.le.problem%n) then
                  select case (problem%n)
                    case (2)
                      read(input_fileunit,*) node(sn)%ctype(k,1), center(1:2), theta
                    case (3)
                      read(input_fileunit,*) node(sn)%ctype(k,1), center, axis, theta
                  end select
                  ...
                  call fbem_error_message(error_unit,0,'node',node(sn)%id,'invalid type of boundary condition for this node')
                end if
            end select
          end do
...
end subroutine read_conditions_fem_nodes_mechanics_harmonic
