...
In this section, the collocation strategies over boundaries can be modified. The general format for the section is:
...
boundary <boundary id>: <type of collocation strategy> <value>
\end{Verbatim} 
\noindent where the types of collocation strategies are given in table~\ref{tab:bem_formulation_boundaries}. 

For instance, in a problem with six boundaries, where the collocation strategy in all boundaries must be nodal, with the exception of boundary 6, where a non-nodal collocation strategies is prefered for all the nodes along its boundaries, with a displacement towards inside each element of 5\% the width of the element, one should write:
\begin{Verbatim}[frame=single, fontsize=\small, label={input.dat}]
...
...
\midrule
\texttt{sbie} & Nodal collocation strategy for all nodes in the boundary. This is unsafe since at doubled nodes, depending on the boundary conditions, it can lead to a singular linear system of equations.\\
\texttt{sbie\_boundary\_mca} & Nodal collocation strategy for all nodes in the boundary except for the nodes along its boundaries, where non-nodal collocation is used. The distance for such non-nodal collocation must be specified in relative terms, within the range (0.01-0.5). \\
\texttt{sbie\_mca} & Non-nodal collocation strategy for all nodes in the boundary. The distance for such non-nodal collocation must be specified in relative terms, within the range (0.01-0.5). \\
\texttt{hbie} & Nodal collocation strategy for all nodes in the boundary. This is unsafe since at doubled nodes, depending on the boundary conditions, it can lead to a singular linear system of equations. Applicable if the hypersingular formulation is used. \\
\texttt{dual\_bm\_same} & Non-nodal collocation strategy for all nodes in the boundary both for the singular and hypersingular formulations (Burton and Miller). The distance for such non-nodal collocation must be specified in relative terms, within the range (0.01-0.5). \\
\texttt{dual\_bm\_mixed} & Nodal collocation strategy for all nodes in the boundary for the singular equations, and non-nodal collocation for the hypersingluar equations only along the boundaries of the boundaries(Burton and Miller). The distance for such non-nodal collocation must be specified in relative terms, within the range (0.01-0.5). \\
\end{tabular}
