...
subroutine assign_default_conditions_bem_boundaries_mechanics_static
      ...
      case (fbem_boundary_coupling_be)
        select case (boundary(i)%class)
          ...
          case (fbem_boundary_class_ordinary)
            ...
            allocate (boundary(i)%cvalue_r(problem%n,7,1))
            ! t_k=0
            boundary(i)%ctype(:,1)=1
            boundary(i)%cvalue_r(:,1,1)=0.d0
          ...
          case (fbem_boundary_class_cracklike)
            ...
            allocate (boundary(i)%cvalue_r(problem%n,7,2))
            ! Face + : t_k=0
            boundary(i)%ctype(:,1)=1
            boundary(i)%cvalue_r(:,1,1)=0.d0
            ! Face - : t_k=0
            boundary(i)%ctype(:,2)=1
            boundary(i)%cvalue_r(:,1,2)=0.d0
...
end subroutine assign_default_conditions_bem_boundaries_mechanics_static
