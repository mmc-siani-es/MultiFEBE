...
sub do_toplevel {
  ...
  print OUT "\n<HR><HR><H2> Description of Types </H2>\n" if @list;
  do_html (@list);
  @list = map (($_->{'type'} eq "var" ? ($_) : ()), @{$top->{'ocontains'}});
  print OUT "\n<HR><HR><H2> Description of Variables </H2>\n" if @list;
  do_html (@list);
  @list = map (($_->{'type'} eq "interface" ? ($_) : ()), @{$top->{'ocontains'}});
  print OUT "\n<HR><HR><H2> Description of Interfaces </H2>\n" if @list;
  do_html (@list);
  @list = map (($_->{'type'} eq "subroutine" || $_->{'type'} eq "function" ? ($_) : ()), @{$top->{'ocontains'}});
  print OUT "\n<HR><HR><H2> Description of Subroutines and Functions </H2>\n" if @list;
  do_html (@list);
...
sub do_html {
   if (@_) {
      ...
      foreach $struct (@_) {
         my ($name, $type) = (txt2html ($struct->{'name'}), $struct->{'type'});
         if (! $htmling::indent) {
            print OUT "<HR><A NAME=\"${type}_" . lc ($name) .
               "\"><H3>$name</H3></A>\n";
            print OUT "<PRE>";
         }
