...
for ke=1:numelemtype
    ...
    for kne=1:numelems
        ...
        system('multifebe -i cylindrical_shell.dat');

        data=readtable('cylindrical_shell.dat.nso', 'NumHeaderLines', 16,'FileType', 'text');
        ...
        data =readtable('cylindrical_shell.dat.eso', 'NumHeaderLines', 21,'FileType', 'text');
        data=table2array(data);
...
for ke=1:numelemtype
    ...
    for kne=1:numelems
        ...
        ylabel('$\mathrm{N_x \ [lb/in]}$','Interpreter', 'latex');
        ax = gca;
        ax.FontSize = fs;
        ...
        ylabel('$\mathrm{M_y \ [lb\cdot in/in]}$','Interpreter','latex')
        ax = gca;
        ax.FontSize = fs;
        ...
        ylabel ('$\mathrm{Q_y \ [lb/in]}$','Interpreter','latex')
        ax = gca;
        ax.FontSize = fs;
    end
