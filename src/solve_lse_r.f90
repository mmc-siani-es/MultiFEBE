...
!! real numbers of <tt>kind=real64</tt> and multiple right hand sides.
subroutine solve_lse_r(n_dof,A,ipiv,Aodim,Ao,equed,r,c,n_rhs,b,factorize,scaling,condition,refine)
  ...
  real(kind=real64)              :: b(n_dof,n_rhs)  !! Vector b
  ...
  ! Local
  real(kind=real64), allocatable :: bcopy(:,:)      !! Copy of vector b required by the refine stage
...
!  !
!  ! Write A,b to ascii file
!  !
!  do i=1,n_dof
!  do j=1,n_dof
!  write(33,'(e25.16)',advance='no') A(i,j)
!  end do
!  write(33,*)
!  end do
!  do i=1,n_dof
!  write(44,'(e25.16)') b(i,1)
!  end do
  ...
  if (scaling) then
    ...
    if (factorize) then
      call dgeequ(n_dof,n_dof,A,n_dof,r,c,rowcnd,colcnd,amax,info)
      if (info.eq.0) then
      ...
      call dlaqge(n_dof,n_dof,A,n_dof,r,c,rowcnd,colcnd,amax,equed)
  ...
  if (factorize) then
    call dgetrf(n_dof,n_dof,A,n_dof,ipiv,info)
    if (info.eq.0) then
      if (verbose_level.ge.2) then
  ...
  trans='N'
  call dgetrs(trans,n_dof,n_rhs,A,n_dof,ipiv,b,n_dof,info)
  if (info.eq.0) then
    if (verbose_level.ge.2) then
