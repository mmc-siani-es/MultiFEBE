...
subroutine build_lse_mechanics_bem_harpot_sa(kf,kr)
  ...
  real(kind=real64), allocatable    :: phi_f1(:,:,:)
  real(kind=real64), allocatable    :: phi_f2(:,:,:)
  real(kind=real64), allocatable    :: x(:,:,:)
  real(kind=real64), allocatable    :: n(:,:,:)
  real(kind=real64), allocatable    :: jw(:,:)
  integer                           :: precalset_ngp(n_precalsets)  ! Number of integration points of each dataset
  integer                           :: precalset_ngp_max
  integer                           :: gln_far, gln_near, gln_qs    ! Gauss-Legendre rules selection variables
  integer                           :: selected_gln                 ! Selected integration Gauss-Legendre rule
  ...
  integer                           :: integration                  ! Integration control: 0: interior, 1: exterior
  ! Region properties
  real(kind=real64)                  :: rho
  complex(kind=real64)               :: c, k
  real(kind=real64)                  :: d1J
  type(fbem_bem_harpot2d_parameters) :: p2d                         ! Parameters of the fluid region
  type(fbem_bem_harpot3d_parameters) :: p3d                         ! Parameters of the fluid region
  ! Incident wave variables
  complex(kind=real64)              :: pi, Uni
  complex(kind=real64), allocatable :: p_inc(:), Un_inc(:)
  real(kind=real64), allocatable    :: xn(:), nn(:)
            ...
            do kn_col=1,se_col_n_nodes
              ...
              if (node(sn_col)%dual_is_common) then
                ...
                if ((node(sn_col)%sbie.eq.fbem_sbie).and.(.not.node_collocated(sn_col))) then
                  ...
                  select case (problem%n)
                    case (2)
                      call fbem_bem_harpot2d_vsbie_auto(se_int_data,se_int_reversion,x_i,p2d,qsi_parameters,qsi_ns_max,h1,h2,g1,g2)
                    case (3)
                      stop 'not yet 69'
                  end select
                ...
                if (node(sn_col)%sbie.eq.fbem_sbie_mca) then
                  ...
                  !
                  if (ks.gt.1) then
                    stop 'symmetry not yet for sensitibility analysi'
                    h1=symconf_s*h1
                    h2=symconf_s*h2
                    g1=symconf_s*g1
                    g2=symconf_s*g2
                ...
                select case (boundary(sb_col)%coupling)
                  case (fbem_boundary_coupling_be,fbem_boundary_coupling_be_fe)
                    select case (boundary(sb_col)%class)
                      case (fbem_boundary_class_ordinary)
                        g1p=g1p*d1J
                        g2p=g2p*d1J
                        call assemble_bem_harpot_equation_sa(omega,kr,sb_int,sb_int_reversion,se_int,se_int_n_nodes,sdme_int_n_nodes,h1p,h2p,g1p,g2p,dme_dxda,dxda_i,sn_col,1)
                  ...
                  case (fbem_boundary_coupling_be_be,fbem_boundary_coupling_be_fe_be)
                    ...
                    if (sb_col_reversion.eqv.(.false.)) then
                      ...
                      call assemble_bem_harpot_equation_sa(omega,kr,sb_int,sb_int_reversion,se_int,se_int_n_nodes,sdme_int_n_nodes,h1p,h2p,g1p,g2p,dme_dxda,dxda_i,sn_col,2)
                    end if
                end select
              end if
...
end subroutine build_lse_mechanics_bem_harpot_sa
