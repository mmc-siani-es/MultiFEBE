...
subroutine assemble_bem_harpot_equation_sa(omega,kr,sb_int,sb_int_reversion,&
  ...
  complex(kind=real64) :: h1(sdme_int_n_nodes,se_int_n_nodes,problem%n)
  complex(kind=real64) :: h2(                 se_int_n_nodes,problem%n)
  complex(kind=real64) :: g1(sdme_int_n_nodes,se_int_n_nodes,problem%n)
  complex(kind=real64) :: g2(                 se_int_n_nodes,problem%n)
  ...
  integer              :: tmp_sn_col
  integer              :: row, col
  complex(kind=real64) :: h(se_int_n_nodes)
        ...
        select case (boundary(sb_int)%class)
          ...
          case (fbem_boundary_class_ordinary)
            ...
            do kn_int=1,se_int_n_nodes
              ...
              bsa_c(row,ka)=bsa_c(row,ka)-h(kn_int)*node(sn_int)%value_c(1,1)
              bsa_c(row,ka)=bsa_c(row,ka)+g(kn_int)*node(sn_int)%value_c(2,1)
            end do
        ...
        if (sb_int_reversion.eqv.(.false.)) then
          ...
          select case (region(boundary(sb_int)%region(1))%type)
            ...
            case (fbem_potential)
              ...
              do kn_int=1,se_int_n_nodes
                sn_int=element(se_int)%node(kn_int)
                bsa_c(row,ka)=bsa_c(row,ka)-h(kn_int)*node(sn_int)%value_c(1,2)
                bsa_c(row,ka)=bsa_c(row,ka)+g(kn_int)*node(sn_int)%value_c(2,2)
              end do
...
end subroutine assemble_bem_harpot_equation_sa
