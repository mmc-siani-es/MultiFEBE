...
subroutine calculate_internal_points_mechanics_bem_harela(kf,kr)
  ...
  do kb_int=1,region(kr)%n_be_bodyloads
    ...
    do ke_int=1,part(sp_int)%n_elements
      se_int=part(sp_int)%element(ke_int)
      se_int_n_nodes=element(se_int)%n_nodes
      call calculate_internal_points_mechanics_bem_harela_bl(omega,kr,sb_int,se_int,se_int_n_nodes,p2d,p3d)
    end do
    !$omp end parallel do
  end do
    ...
    do kip=1,region(kr)%n_internalpoints
      ...
      do il=1,problem%n
        do ik=1,problem%n
          do kn_int=1,se_int_n_nodes
            sn_int=element(se_int)%node(kn_int)
            internalpoint(sip)%value_c(il,0)=internalpoint(sip)%value_c(il,0)+g(kn_int,il,ik)*node(sn_int)%value_c(problem%n+ik,1)
          end do
        end do
      end do
      !$omp end critical
      ...
      do kc=1,problem%n
        ...
        do il=1,problem%n
          do ik=1,problem%n
            do kn_int=1,se_int_n_nodes
              sn_int=element(se_int)%node(kn_int)
              internalpoint(sip)%value_c(il,kc)=internalpoint(sip)%value_c(il,kc)+l(kn_int,il,ik)*node(sn_int)%value_c(problem%n+ik,1)
            end do
          end do
        end do
        !$omp end critical
...
end subroutine calculate_internal_points_mechanics_bem_harela_bl
