  ...
  if (status_ok) then
    if (kf.eq.1) then
      ...
      call file_csv%add(['Frequency index','Frequency value ['//trim(freq_str)//']',&
                         trim(complex_str1)//'(c)',trim(complex_str2)//'(c)'],trim_str=.true.)
    ...
    if (complex_notation.eq.2) then
      call file_csv%add(dreal(c),real_fmt='('//fmt_real//')')
      call file_csv%add(dimag(c),real_fmt='('//fmt_real//')')
    else
      call file_csv%add(      abs(c),real_fmt='('//fmt_real//')')
      call file_csv%add(fbem_zarg(c),real_fmt='('//fmt_real//')')
    end if
    call file_csv%next_row()
  else
