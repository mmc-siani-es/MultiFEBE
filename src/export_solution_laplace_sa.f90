...
subroutine export_solution_laplace_sa
      ...
      do kr=1,n_regions
        ...
        select case (region(kr)%class)
                      ...
                      case (fbem_boundary_coupling_be,fbem_boundary_coupling_be_fe)
                        select case (boundary(sb)%class)
                          ...
                          case (fbem_boundary_class_ordinary)
                            ...
                            ! Write columns >=10
                            write(output_fileunit,fmt2,advance='no') node(sn)%dvda_r(1,face,ka)
                            write(output_fileunit,fmt2,advance='no') node(sn)%dvda_r(2,face,ka)
                            write(output_fileunit,*)
                          !
          ...
          do kip=1,region(kr)%n_internalpoints
            ...
            ! PRIMARY VARIABLES
            write(output_fileunit,fmt2,advance='no') internalpoint(sip)%dvda_r(1,0,ka)
            ! SECONDARY VARIABLES
            do kc=1,problem%n
              write(output_fileunit,fmt2,advance='no') internalpoint(sip)%dvda_r(1,kc,ka)
            end do
            write(output_fileunit,*)
          end do
...
end subroutine export_solution_laplace_sa
