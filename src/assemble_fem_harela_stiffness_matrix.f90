...
subroutine assemble_fem_harela_stiffness_matrix(se,se_n_dof,K)

  !
  ! Igual que la estatica, pero con los siguientes cambios:
  !
  ! 1) K() y Kt() deben ser complejas
  ! 2) A_r -> A_c
  ! 3) b_r -> b_c
  ! 4) cvalue_r -> cvalue_c
  !
  ...
  complex(kind=real64)              :: K(se_n_dof,se_n_dof) !! Stiffness matrix
  ...
  complex(kind=real64), allocatable :: Kt(:,:)
                ...
                if (node(snr)%ctype(kcr,1).eq.1) then
                  ...
                  do kcc=1,element(se)%node_n_dof(knc)
                    ...
                    if (node(snc)%ctype(kcc,1).eq.1) then
                      ...
                      A_c(row,col)=A_c(row,col)+K(row_local,col_local)
                    else
                      b_c(row,1)=b_c(row,1)-K(row_local,col_local)*node(snc)%cvalue_c(kcc,1,1)
                    end if
                  end do
                end if
              end do
                ...
                if (node(snr)%ctype(kcr,1).eq.1) then
                  ...
                  do kcc=1,node(sncm)%n_dof
                    if (node(sncm)%ctype(kcc,1).eq.1) then
                      ...
                      A_c(row,col)=A_c(row,col)+Kt(kcr,kcc)
                    else
                      b_c(row,1)=b_c(row,1)-Kt(kcr,kcc)*node(sncm)%cvalue_c(kcc,1,1)
                    end if
                  end do
                end if
              end do
                ...
                if (node(snrm)%ctype(kcr,1).eq.1) then
                  ...
                  do kcc=1,element(se)%node_n_dof(knc)
                    if (node(snc)%ctype(kcc,1).eq.1) then
                      ...
                      b_c(row,1)=b_c(row,1)-Kt(kcr,kcc)*node(snc)%cvalue_c(kcc,1,1)
                    end if
                  end do
                end if
              end do
...
end subroutine assemble_fem_harela_stiffness_matrix
...
!~             ! The variable is ACTIVE if the associated DOF is ACTIVE
!~             if (node(snc)%ctype(kcc,1).eq.1) then
!~               ! COLUMN of the VARIABLE in A
!~               col=node(snc)%col(kcc,1)
!~               A_c(row,col)=A_c(row,col)+K(row_local,col_local)
!~             else
!~               b_c(row,1)=b_c(row,1)-K(row_local,col_local)*node(snc)%cvalue_c(kcc,1,1)
