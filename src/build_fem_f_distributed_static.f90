...
subroutine build_fem_f_distributed_static(se,ndof_fe,f_e)
  ...
  integer                        :: se
  integer                        :: ndof_fe
  real(kind=real64)              :: f_e(ndof_fe)
  ! Local variables
  integer                        :: ndof_se, ndof_load
  real(kind=real64), allocatable :: Q(:,:), t_e(:), Ld(:,:), nodal_axes(:,:,:)
  integer                        :: sr, kn, kc
  real(kind=real64)              :: nu
          ...
          if ((element(se)%ctype(1,1).gt.0).or.(element(se)%ctype(2,1).gt.0)) then
            ...
            if (element(se)%ctype(1,1).gt.0) then
              ...
              do kn=1,element(se)%n_nodes
                t_e((problem%n*(kn-1)+1):(problem%n*kn))=element(se)%cvalue_r(:,kn,1)
              end do
            ...
            if (element(se)%ctype(2,1).gt.0) then
              t_e=0.d0
              do kn=1,element(se)%n_nodes
                t_e((problem%n*(kn-1)+1):(problem%n*kn))=element(se)%cvalue_r(:,kn,2)
              end do
...
!~               t_e=0.d0
!~               do kn=1,element(se)%n_nodes
!~                 t_e((3*(problem%n-1)*(kn-1)+1):(3*(problem%n-1)*kn-(2*problem%n-3)))=element(se)%cvalue_r(:,kn,2)
!~               end do
!~               f_e(1:ndof_se)=f_e(1:ndof_se)+matmul(Q,t_e)
!~             end if
!~             deallocate (Q,t_e)
        ...
        case (3)
          ...
          if ((element(se)%ctype(1,1).ne.0).or.(element(se)%ctype(2,1).ne.0)) then
            ...
            if (element(se)%ctype(1,1).eq.1) then
              t_e=0.d0
              do kn=1,element(se)%n_nodes
                t_e((3*(kn-1)+1):(3*kn))=element(se)%cvalue_r(:,kn,1)
              end do
              f_e(1:ndof_se)=f_e(1:ndof_se)+matmul(Q,matmul(Ld,t_e))
            end if
            ...
            if (element(se)%ctype(2,1).eq.1) then
              t_e=0.d0
              do kn=1,element(se)%n_nodes
                t_e((3*(kn-1)+1):(3*kn))=element(se)%cvalue_r(:,kn,2)
              end do
              f_e(1:ndof_se)=f_e(1:ndof_se)+matmul(Q,t_e)
            end if
            ! Deallocate
...
end subroutine build_fem_f_distributed_static
